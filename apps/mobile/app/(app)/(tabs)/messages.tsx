...
export default function Messages() {
  ...
  if (chats.loading && !chats.data) return <LoadingState label="Loading messages..." />;
  return (
    <Screen>
      <Text accessibilityRole="header" style={{ color: theme.text, fontSize: 24, fontWeight: '800' }}>Messages</Text>
      ...
      {chats.data?.chats?.length ? chats.data.chats.map((chat) => (
        <Link key={chat._id || chat.id} href={`/chats/${chat._id || chat.id}`} style={{ color: theme.text, padding: 12, borderWidth: 1, borderColor: theme.border, borderRadius: 8 }}>
          {chat.name || chat.title || chat.type || 'Chat'}
        </Link>
      )) : <EmptyState title="No chats" body="Your authorized conversations will appear here." />}
    </Screen>
  );
}
